...
//Shanghai Location Markers
    ...
    ["<h3 class=info-window-styling>Hakkasan</h3> <a class=info-window-link target=_blank href=https://hakkasan.com/shanghai/>Website</a> <p class=info-window-p>18 Zhongshan East 1st Rd, Nan Jing Dong Lu, Huangpu, Shanghai, China</P> <button class=info-window-button onclick=choosePlace('hakkasan')>Find Out More</button>", 31.23863, 121.48971, "restaurants"],
...
// Beijing Location Markers
    ["<h3 class=info-window-styling>Temple Of Heaven</h3> <p class=info-window-p>1 Tiantan E Rd, Dongcheng, China, 100061</P> <button class=info-window-button onclick=choosePlace('templeOfHeaven')>Find Out More</button>", 39.8822, 116.4066, "attractions"],
    ["<h3 class=info-window-styling>Imperial Palace & The Foribdden City</h3> <p class=info-window-p>4 Jingshan Front St, Dongcheng, Beijing, China, 100886</P> <button class=info-window-button onclick=choosePlace('theImperialPalaceAndTheForbiddenCity')>Find Out More</button>", 39.9169, 116.3907, "attractions"],
    ["<h3 class=info-window-styling>The Great Wall of China</h3> <p class=info-window-p>Huairou District, China</P> <button class=info-window-button onclick=choosePlace('theGreatWallOfChina')>Find Out More</button>", 40.4319, 116.5704, "attractions"],
    ["<h3 class=info-window-styling>Beijing Capital Museum</h3> <p class=info-window-p>16 Fuxingmen Outer St, Xicheng District, China, 100035</P> <button class=info-window-button onclick=choosePlace('beijingCapitalMuseum')>Find Out More</button>", 39.9064, 116.3419, "attractions"],
    ["<h3 class=info-window-styling>Jubaoyuan</h3> <p class=info-window-p>5-2 Niujie St, Xicheng District, Beijing, China</P> <button class=info-window-button onclick=choosePlace('jubaoyuanHotPotRestaurant')>Find Out More</button>", 39.8869, 116.3632, "restaurants"],
    ["<h3 class=info-window-styling>Qing-Feng Steamed Dumpling</h3> <p class=info-window-p>Dongdan N St, Wangfujing, Dongcheng, Beijing, China</P> <button class=info-window-button onclick=choosePlace('qingFendSteamedDumplingShop')>Find Out More</button>", 39.5527, 116.2320, "restaurants"], 
    ["<h3 class=info-window-styling>Hai Wan Ju</h3> <p class=info-window-p>China, Beijing, Chaoyang, Songyu S Rd</P> <button class=info-window-button onclick=choosePlace('haiwanju')>Find Out More</button>", 39.87126, 116.47508, "restaurants"],
    ["<h3 class=info-window-styling>Beihai Park</h3> <p class=info-window-p>1 Wenjin St, Xicheng District, China, 100034</P> <button class=info-window-button onclick=choosePlace('beihaiPark')>Find Out More</button>", 39.92401, 116.40750, "restaurants"],
    ["<h3 class=info-window-styling>Tiananmen Square</h3> <p class=info-window-p>Dongcheng, China</P> <button class=info-window-button onclick=choosePlace('tianmentSquare')>Find Out More</button>", 39.9254, 116.3893, "parks"],
    ["<h3 class=info-window-styling>Coal Hill Park</h3> <p class=info-window-p>Woodstock La North, Surbiton KT6 5HN</P> <button class=info-window-button onclick=choosePlace('coalHillPark')>Find Out More</button>", 39.5525, 116.2326, "parks"],
    ["<h3 class=info-window-styling>The Old Summer Palace</h3> <p class=info-window-p>Haidian District, China, 100084</P> <button class=info-window-button onclick=choosePlace('theOldSummerPalace')>Find Out More</button>", 40.0081, 116.2982, "parks"],
// Hong Kong Location Markers
    ["<h3 class=info-window-styling>Victoria Peak</h3> <p class=info-window-p>The Peak, Hong Kong</P> <button class=info-window-button onclick=choosePlace('victoriaPeak')>Find Out More</button>", 22.2759, 114.1455, "attractions"],
    ...
    ["<h3 class=info-window-styling>Big Buddha</h3> <p class=info-window-p>Ngong Ping Rd, Lantau Island, Hong Kong</P> <button class=info-window-button onclick=choosePlace('bigBuddha')>Find Out More</button>", 22.2540, 113.9050, "attractions"],
    ["<h3 class=info-window-styling>Wong Tai Sin Temple</h3> <p class=info-window-p>2 Chuk Yuen Rd, Chuk Un, Hong Kong</P> <button class=info-window-button onclick=choosePlace('wongTaiSinTemple')>Find Out More</button>", 22.3427, 114.1933, "attractions"],
    ["<h3 class=info-window-styling>Aulis</h3> <p class=info-window-p>UG08, Sino Plaza, 255 Gloucester Rd, Causeway Bay, Hong Kong</P> <button class=info-window-button onclick=choosePlace('aulis')>Find Out More</button>", 22.2815, 114.1824, "restaurants"],
    ["<h3 class=info-window-styling>Chaat</h3> <p class=info-window-p>5/F Rosewood Hong Kong, Victoria Dockside, 18 Salisbury Rd, Tsim Sha Tsui, Hong Kong</P> <button class=info-window-button onclick=choosePlace('chaat')>Find Out More</button>", 22.2949, 114.1755, "restaurants"],
    ["<h3 class=info-window-styling>Arbor</h3> <p class=info-window-p>Hong Kong, Central, Queen's Road Central</P> <button class=info-window-button onclick=choosePlace('arbor')>Find Out More</button>", 22.28322, 114.15535, "restaurants"],
    ["<h3 class=info-window-styling>The Chairman</h3> <p class=info-window-p>18 Kau U Fong, Central, Hong Kong</P> <button class=info-window-button onclick=choosePlace('theChairman')>Find Out More</button>", 22.284580, 114.152672, "restaurants"],
    ["<h3 class=info-window-styling>Ocean Park</h3> <p class=info-window-p>Aberdeen, Hong Kong</P> <button class=info-window-button onclick=choosePlace('oceanPark')>Find Out More</button>", 22.2467, 114.1757, "parks"],
    ["<h3 class=info-window-styling>Hong Kong Park</h3> <p class=info-window-p>Central, Hong Kong</P> <button class=info-window-button onclick=choosePlace('hongKongPark')>Find Out More</button>", 22.2765, 114.1596, "parks"],
    ["<h3 class=info-window-styling>Chi Lin Nunnery and Nan</h3> <p class=info-window-p>Chi Lin Nunnery, 5 Chi Lin Dr, Sheung Yuen Leng, Hong Kong</P> <button class=info-window-button onclick=choosePlace('chiLinNunneryAndNanLianGarden')>Find Out More</button>", 22.3404, 114.2055, "parks"],
    ["<h3 class=info-window-styling>Hong Kong Skyline</h3> <p class=info-window-p>No. 1 Austin Road, West Kowloon</P> <button class=info-window-button onclick=choosePlace('hongKongSkyline')>Find Out More</button>", 22.2855, 114.1577, "parks"]
];
